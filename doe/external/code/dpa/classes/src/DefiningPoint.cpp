...
IloCplex createCplex(IloEnv env) {
    ...
    //result.setParam(IloCplex::Param::Emphasis::Numerical, 1); // emphasize numerical accuracy
	return result;
}
...
bool DefiningPoint::SolveAugmentedEconstraint(int indexEc, const int* rhs, int* obj) {
    ...
    statusMessages.push_back(to_string(cplex.getCplexStatus()));
...
void DefiningPoint::ExportNonDominatedPointsToFile(const std::string fileName) {
    ...
    string fileVars = fileName + "_vars.txt";
    ofstream outputV(fileVars, ofstream::out);
    for (int i = 0; i < variables.getSize(); i++) {
        outputV << variables[i] << endl;
    }
    outputV.close();

    string fileTermination = fileName + "_termination.txt";
    ofstream outputT(fileTermination, ofstream::out);
    for (int i = 0; i < statusMessages.size(); i++) {
        outputT << statusMessages[i] << endl;
    }
    outputT.close();
