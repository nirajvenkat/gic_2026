...
# the spectrum of the $\mathrm{H}_2\mathrm{O}$ molecule with a small active space.
...
mol = gto.Mole(atom=list(zip(symbols, geometry)), basis="sto-3g", symmetry=None)
mol.build(verbose=0)
...
# electrons. For $\mathrm{H}_2\mathrm{O}$, we will use an active space of seven
# orbitals and ten electrons. Running a `CASCI` instance allows us to
...
n_cas, n_electron_cas = (7, 10)

# Number of core (non-active) orbitals.
n_core = (mol.nelectron - n_electron_cas) // 2  

# Initialize CASCI instance of H2O molecule as mycasci.
...
# Define range of valence transition energies in eV (10 eV to 100 eV)
wgrid_ev = np.linspace(10.0, 100.0, 10000)
wgrid = E_i + wgrid_ev / 27.211386  # Convert to absolute energies in Hartree
w_min, w_step = wgrid[0], wgrid[1] - wgrid[0]

spectrum = np.array([f_domain_Greens_func(w) for w in wgrid])

# %% [markdown]
# Since our chosen active space for $\mathrm{H}_2\mathrm{O}$ is small, we can easily
...
mycasci.fcisolver.nroots = 250  # Compute the first 250 states.
mycasci.run(verbose=0)
...
ax.plot((wgrid - E_i) * 27.211386, spectrum, label="quantum")
ax.plot((wgrid - E_i) * 27.211386, spectrum_classical, "--", label="classical")
ax.set_xlabel(r"$\mathrm{Energy}, \omega\ (\mathrm{eV})$")
ax.set_ylabel(r"$\mathrm{Absorption\ (arb.)}$")
ax.legend()
fig.text(0.5, 0.05,
    r"Figure 8: Simulated absorption spectrum for $\mathrm{H}_2\mathrm{O}$ active space.",
    horizontalalignment="center",
    size="small", weight="normal")
plt.show()
