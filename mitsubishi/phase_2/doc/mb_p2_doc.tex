...
The model is trained using AdamW to minimize the loss $\mathcal{L} = \frac{1}{B} \sum_{b=1}^B \sum_{t=1}^L \left( \sum_{i=1}^t \text{logit}_{j_i} - E(\mathbf{j}_{1:t}) \right)^2 + \beta \sum_{t=1}^L \max(0, E_{\rm FCI} - E(\mathbf{j}_{1:t}))^2$, where the penalty parameter $\beta = 10.0$ prevents the predicted energy values from dipping below the exact Full Configuration Interaction (FCI) ground-state energy.
...
\end{enumerate}
We solve the equation-of-motion secular equation under the double commutator approximation, $\langle 0 | [\delta O, [\hat{H}, O_n^\dagger]] | 0 \rangle = \omega_n \langle 0 | [\delta O, O_n^\dagger] | 0 \rangle$ (where $\delta O$ is a variation of the excitation operator), in an STO-3G basis set to obtain the transition energies $\omega_n$ and excited state energies $E_I$ and $E_K$.
